    ...
    def compare(self, encoded_faces_to_compare, encoded_reference_faces):
        ...
        for face in encoded_faces_to_compare:
            for name in self.names:
                ...
                same_person = fr.compare_faces(encoded_reference_faces[name][0], encoded_faces_to_compare[face], tolerance=0.57) #returns a list of booleans
                if any(same_person):
                    self.recognised_people.append(name)
            if self.recognised_people:
                self.file_name_binder[face] = self.recognised_people #puts face in recognised people for this file
                self.recognised_people = []
